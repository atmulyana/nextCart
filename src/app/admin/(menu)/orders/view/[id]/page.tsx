...
export default async function ViewOrder(props: {params: Promise<{id: string}>}) {
    ...
    return <Template>
        ...
        <ul className='bordered'>
            ...
            <li className='bg-[var(--bg-color)]'>&nbsp;</li>
            <li className='bg-[var(--bg-color)] text-blue-500'>{lang('Products ordered')}</li>
            {Object.values(order.orderProducts).map((p, i) => 
            <li key={i} className='bg-[var(--bg-color)]'>
                {p.quantity} x {p.title}
                {p.variantId && <>
                    &nbsp; &gt; &nbsp;
                    <span className="text-[var(--color-warning)]">{lang('Option')}:&nbsp;</span>
                    {p.variantTitle}
                </>}
                <div className="float-right">{currencySymbol()}{formatAmount(p.totalItemPrice)}</div>
                {p.productComment && <h4 className='text-[var(--color-danger)]'>{p.productComment}</h4>}
            </li>)}
        </ul>
    </Template>;
}