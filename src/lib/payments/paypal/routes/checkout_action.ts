...
export const POST = createPostHandler(async (_, redirect, isFromMobile) => {
    ...
    //if (!cart) return await redirect('/', {message: lang('There are no items in your cart. Please add some items before checking out.')});
    ...
    else return await redirect('/checkout/payment', {message: obj.message || ''});
});